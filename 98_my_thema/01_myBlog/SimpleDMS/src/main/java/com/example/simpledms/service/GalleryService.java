...
public class GalleryService {
    ...
    public Optional<Gallery> getFile(int fid) {
        Optional<Gallery> optionalGallery = galleryRepository.findById(fid);

        return optionalGallery;
    }

    //    galleryTitle(이미지명) 으로 like 검색하는 함수
    public Page<Gallery> findAllByFileTitleContaining(String galleryTitle, Pageable pageable) {
    ...
    public boolean removeById(int fid) {
        ...
        if (galleryRepository.existsById(fid) == true) {
            galleryRepository.deleteById(fid); // 삭제 실행
            return true;
        }

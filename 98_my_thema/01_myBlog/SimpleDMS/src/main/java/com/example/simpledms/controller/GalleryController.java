...
//          Upload 이미지 정보를 가져오는 함수
            Page<ResponseGalleryDto> filePage = galleryService
                                      .findAllByFileTitleContaining(galleryTitle, pageable)
                    .map(dbFile -> {
//                      자동적으로 반복문이 실행됨 : .map()
//                      1) 다운로드 URL 만들기
...
//                               .fromCurrentRequest() -> .fromCurrentContextPath()
                                ...
                                .path("/api/gallery/")
                                .path(dbFile.getGid().toString()) // "/api/gallery/1"
...
//            맵 자료구조에 넣어서 전송
            ...
            response.put("gallery", filePage.getContent());

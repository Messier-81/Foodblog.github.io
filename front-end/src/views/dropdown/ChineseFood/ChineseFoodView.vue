    ...
    <div>
        ...
        <div class="container-fluid page-header py-6 wow fadeIn" data-wow-delay="0.1s">
            <div class="container text-center pt-5 pb-3">
                <h1 class="display-4 text-white animated slideInDown mb-3">중식</h1>
                <nav aria-label="breadcrumb animated slideInDown">
                    <ol class="breadcrumb justify-content-center mb-0">
                        <li class="breadcrumb-item"><a class="text-white" href="/">Home</a></li>
                        <li class="breadcrumb-item"><a class="text-white" href="#">Pages</a></li>
                        <li class="breadcrumb-item text-primary active" aria-current="page">중식</li>
                    </ol>
                </nav>
            </div>
        </div>
...
retrieveChineseFood();
            ...
            <table class="table">
                ...
                <tbody v-for="(data, index) in chineseFood" :key="index">
                    <tr>
                        <td>{{ data.title }}</td>
                        <!-- <!— <td>{{ data.lastName }}</td> —> -->
                        <td>{{ data.email }}</td>
                        <td>{{ data.phone }}</td>
                        <td>
                            <router-link :to="'/chineseFood/' + data.cno"><span
                                    class="badge rounded-pill bg-warning text-dark">Edit</span>
                            </router-link>
                        </td>
                    </tr>
                </tbody>
            </table>
            <router-link to="/add-chineseFood/">
...
export default {
    ...
    methods: {
        // 함수명   : retrieveChineseFood()
        // 매개변수 : this.searchEmail (email 검색어)
        //            this.page (현재 페이지 번호)
        //            this.pageSize (페이지 당 개수)
        // 리턴값 : 없음
        retrieveChineseFood() {
            ChineseFoodDataService.getAll(this.searchEmail, this.page - 1, this.pageSize)
                .then((response) => {
                    // 모던 자바 문법 : 구조 분해 할당
                    // const(let) {속성, 속성2} = 객체
                    const { chineseFood, totalItems } = response.data

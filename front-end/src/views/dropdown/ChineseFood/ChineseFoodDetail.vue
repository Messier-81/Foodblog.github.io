    ...
    <div>
        ...
        <div class="container-fluid page-header py-6 wow fadeIn" data-wow-delay="0.1s">
            <div class="container text-center pt-5 pb-3">
                <h1 class="display-4 text-white animated slideInDown mb-3">중국음식 갤러리</h1>
        ...
        <!-- 한국 음식 게시판 시작 -->
        <div class="container" v-if="currentChineseFood">
            <div class="mb-3">
                <label for="title" class="form-label">Title</label>
                <input type="text" class="form-control" id="title" required name="title"
                    v-model="currentChineseFood.title" />
            </div>
            <div class="mb-3">
                <label for="email" class="form-label">Email</label>
                <input type="email" class="form-control" id="email" required name="email"
                    v-model="currentChineseFood.email" />
            </div>
            <div class="mb-3">
                <label for="phone" class="form-label">Phone</label>
                <input type="text" class="form-control" id="phone" required name="phone"
                    v-model="currentChineseFood.phone" />
            </div>
            <div class="mb-3">
                <button @click="updateChineseFood" class="btn btn-primary me-3">
                    Update
                </button>
                <button @click="deleteChineseFood" class="btn btn-danger">Delete</button>
            </div>
...
import ChineseFoodDataService from "../../../services/ChineseFoodDataService";
export default {

    data() {
        return {
            currentChineseFood: null,
            message: "",
        };
    },
    methods: {
        // 고객번호(cno)로 조회 요청하는 함수
        getChineseFood(cno) {
            // axios 공통함수 호출
            ChineseFoodDataService.get(cno)
                // 성공하면 .then() 결과가 리턴됨
                .then(response => {
                    // springboot 결과를 리턴함(부서 객체)
                    this.currentChineseFood = response.data;
                    // 콘솔 로그 출력
        ...
        updateChineseFood() {
            // axios 공통함수 호출
            ChineseFoodDataService.update(this.currentChineseFood.cno, this.currentChineseFood)
                // 성공하면 then() 결과가 전송됨
                .then(response => {
                    console.log(response.data);
                    this.message = "The ChineseFood was updated successfully!";
                })
        ...
        deleteChineseFood() {
            // axios 공통함수 호출
            ChineseFoodDataService.delete(this.currentChineseFood.cno)
                // 성공하면 then() 결과가 전송됨
                .then(response => {
                    console.log(response.data);
                    // 첫페이지(전체목록_조회_페이지) 강제 이동 : /dept 
                    this.$router.push("/chineseFood");
                })
                // 실패하면 .catch() 에러메세지가 전송됨
                .catch(e => {
                    console.log(e);
    ...
    mounted() {
        this.message = '';
        this.getChineseFood(this.$route.params.cno);
        myBlog();
    },
}
</script>

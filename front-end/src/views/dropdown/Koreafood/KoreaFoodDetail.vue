<template>
    <div>
        ...
        <!-- 한국 음식 게시판 시작 -->
        <div class="container" v-if="currentKoreaFood">
            <div class="mb-3">
                <label for="title" class="form-label">Title</label>
                <input type="text" class="form-control" id="title" required name="title"
                    v-model="currentKoreaFood.title" />
            </div>
            <div class="mb-3">
                <label for="email" class="form-label">Email</label>
                <input type="email" class="form-control" id="email" required name="email"
                    v-model="currentKoreaFood.email" />
            </div>
            <div class="mb-3">
                <label for="phone" class="form-label">Phone</label>
                <input type="text" class="form-control" id="phone" required name="phone"
                    v-model="currentKoreaFood.phone" />
            </div>
            <div class="mb-3">
                <button @click="updateKoreaFood" class="btn btn-primary me-3">
                    Update
                </button>
                <button @click="deleteKoreaFood" class="btn btn-danger">Delete</button>
            </div>
...
export default {
    ...
    data() {
        return {
            currentKoreaFood: null,
            message: "",
        };
    },
    methods: {
        // 고객번호(kno)로 조회 요청하는 함수
        getKoreaFood(kno) {
            // axios 공통함수 호출
            KoreaFoodDataService.get(kno)
                // 성공하면 .then() 결과가 리턴됨
                .then(response => {
                    // springboot 결과를 리턴함(부서 객체)
                    this.currentKoreaFood = response.data;
                    // 콘솔 로그 출력
        ...
        updateKoreaFood() {
            // axios 공통함수 호출
            KoreaFoodDataService.update(this.currentKoreaFood.kno, this.currentKoreaFood)
                // 성공하면 then() 결과가 전송됨
                .then(response => {
                    console.log(response.data);
                    this.message = "The KoreaFood was updated successfully!";
                })
        ...
        deleteKoreaFood() {
            // axios 공통함수 호출
            KoreaFoodDataService.delete(this.currentKoreaFood.kno)
                // 성공하면 then() 결과가 전송됨
                .then(response => {
                    console.log(response.data);
                    // 첫페이지(전체목록_조회_페이지) 강제 이동 : /dept 
                    this.$router.push("/koreaFood");
                })
                // 실패하면 .catch() 에러메세지가 전송됨
                .catch(e => {
                    console.log(e);
    ...
    mounted() {
        this.message = '';
        this.getKoreaFood(this.$route.params.kno);
        myBlog();
    },
}
</script>

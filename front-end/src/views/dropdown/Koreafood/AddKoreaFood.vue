            ...
            <div v-if="!submitted">
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control" id="title" required name="title"
                        v-model="koreaFood.title" />
                </div>
                <div class="mb-3">
                    <label for="email" class="form-label">Email</label>
                    <input type="email" class="form-control" id="email" required name="email"
                        v-model="koreaFood.email" />
                </div>
                <div class="mb-3">
                    <label for="phone" class="form-label">Phone</label>
                    <input type="text" class="form-control" id="phone" required name="phone"
                        v-model="koreaFood.phone" />
                </div>
                <div class="mb-3">
                    <button @click="saveKoreaFood" class="btn btn-primary">Submit</button>
                </div>
            </div>
            <div v-else>
                <div class="alert alert-success" role="alert">Save koreaFood successfully!</div>
                <button @click="newKoreaFood" class="btn btn-primary">Add New KoreaFood</button>
                <button @click="returnKoreaFood" class="btn btn-primary ms-3">Return KoreaFood</button>
            </div>
        </div>
        <!-- 한국 음식 게시판 끝 -->
...
export default {
    ...
    methods: {
        saveKoreaFood() {
            let data = {
                title: this.koreaFood.title,
                email: this.koreaFood.email,
                phone: this.koreaFood.phone,
            };

            // insert 요청 함수 호출(axios 공통함수 호출)
            KoreaFoodDataService.create(data)
                // 성공하면 then() 결과가 전송됨
                .then((response) => {
                    this.koreaFood.kno = response.data.kno;
